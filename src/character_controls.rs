...
impl MovementBundle {
    pub const fn new(damping: Scalar, jump_impulse: Scalar, max_slope_angle: Scalar) -> Self {
        Self {
...
fn movement(
    ...
    mut jump_counter: ResMut<DoubleJumpCounter>,
    mut controllers: Query<(&JumpImpulse, &mut LinearVelocity, &Transform)>,
) {
    ...
    for event in movement_reader.read() {
        for (jump_impulse, mut linear_velocity, transform) in &mut controllers {
            match event {
